...
const init = async () => {
    ...
    rows.splice(0, 1); // Borrar en el indice 0 un renglon
    const document = new Dato(); // Crear un objeto de tipo dato
    const resultset = [];
    for (const row of rows) {
        const cpCiudadEstado = row.substring(0, 37).trim().split(',');
        if (cpCiudadEstado.length !== 2)
            break;

        document.codigoPostal = cpCiudadEstado[0].substring(0, 5);
        ...
        resultset.push(new Dato(document));
    }
...
/**
 * Almacena un resultset dado del archivo TXT
 * @param {Dato[]} resultset
 */
const saveCsv = async (resultset) => {
    ...
    fs.writeFileSync('datos.csv', csvData.join('\n'));
}
